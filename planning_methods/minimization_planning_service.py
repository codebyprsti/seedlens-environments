"""
Minimization Planning Service (v2.0)

This module implements cost minimization/optimization logic for supply chain planning.
It provides a dedicated service for generating minimized-cost plans by allocating
production to the lowest-cost locations first.

Similar structure to Linear_programming.py but uses greedy cost minimization
instead of two-stage linear programming.
"""
...
class MinimizationPlanner:
    def __init__(self, db: Session):
        """
        Initialize the Minimization Planner with database connection
    ...
    def allocate_target_production_minimization(self, village_summary: pd.DataFrame):
        """
        Allocate target production per state-variety using cost minimization (greedy algorithm).
        Sorts by cost per kg (ascending) and allocates to lowest-cost locations first within each state-variety group.
        ...
        for state, variety_targets in self.state_variety_targets_kgs.items():
            for variety_us_code, target_kgs in variety_targets.items():
                ...
                planned_mask = (
                    ...
                    (state_variety_villages['estimated_cost_per_kg'] > 0)  # Must have valid cost
                )
                ...
                if planned_villages_for_allocation.empty:
                    state_variety_stats.append({
                        ...
                        'status': 'No planned villages with positive productivity and cost'
                    })
                    continue
        
        # Sort by cost per kg (ascending) - lowest cost first
        ...
        # Greedy allocation: allocate to lowest-cost locations first within this state-variety
        allocated = 0.0
        total_cost = 0.0
        total_acres = 0.0
        ...
        for idx, row in planned_villages_for_allocation.iterrows():
            ...
            # Skip if no available acres or productivity
            if available_acres <= 0 or productivity <= 0:
                continue
            ...
            if max_allocation > 0:
                ...
                state_variety_stats.append({
                    ...
                    'num_villages': len(planned_villages_for_allocation),
    ...
    def save_plan_to_database(self, plan_data: pd.DataFrame, plan_year: int, plan_version: str = None) -> bool:
        ...
        Args:
            ...
            plan_version: Version string for the plan (defaults to v2.0-Minimized-Cost)
        ...
        try:
            ...
            if not plan_version:
                plan_version = f"v2.0-Minimized-Cost"
    ...
    def generate_plan(self, plan_year: int):
        ...
        print(f"\nGenerating minimization plan for {plan_year} (Y0: {y0}, Y+1: {y1})")
        ...
        final_plan = self.allocate_target_production_minimization(village_summary)
        ...
        plan_version = f"v2.0-Minimized-Cost"
        logger.info(f"\nSaving minimization plan to database...")
        logger.info(f"  Plan Version: {plan_version}")
        logger.info(f"  Plan Year: {plan_year}")
        ...
        if save_success:
            logger.info(f"Minimization plan successfully saved to database!")
        else:
            logger.error(f" ERROR: Failed to save minimization plan to database!")
    ...
    def print_state_variety_statistics(self, plan_year: int):
        ...
        if self.output_folder and self.output_folder != tempfile.gettempdir():
            stats_filename = os.path.join(self.output_folder, f"state_variety_statistics_minimization_{plan_year}.csv")
...
def main():
    """
    Main function to generate cost minimization supply chain plans
    Loads data from database (schema: operations) only
    """
    print("=" * 80)
    print("SUPPLY CHAIN PLANNING - COST MINIMIZATION (v2.0)")
    ...
    try:
        ...
        planner = MinimizationPlanner(db=db)
        ...
        try:
            ...
            for plan_year in [2024, 2025]:
                verification = planner.verify_saved_plan(plan_version="v2.0-Minimized-Cost", plan_year=plan_year)
            ...
            print("   SELECT COUNT(*) FROM operations.supply_chain_planning WHERE plan_revision_version = 'v2.0-Minimization';")
            print("   SELECT * FROM operations.supply_chain_planning WHERE plan_revision_version = 'v2.0-Minimization' ORDER BY created_at DESC LIMIT 10;")
