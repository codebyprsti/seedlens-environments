"""
Supply Chain Planning Service (v1.0)

This module implements the SQL planning logic from SW_planning_formula_refactored.sql
in Python to automatically generate supply chain planning data when new entries
are added to season_crop_inspection_final table.

Similar structure to Linear_programming.py but uses productivity-based allocation
(maximize yield by allocating to highest productivity locations first).
"""
...
class SupplyChainPlanner:
    def __init__(self, db: Session):
        ...
        # Define target production per state per variety for 2024-2025 plan
        # Values are in MT, will be converted to Kgs (multiply by 1000)
        # Format: {state: {variety: target_MT}}
        # TOTAL TARGET: 10,000 MT (50+3360+100+626+80+1472+1000+150+1200+402+1560)
        ...
        # Define target production per state per variety for 2025-2026 plan
        # Values are in MT, will be converted to Kgs (multiply by 1000)
        # Format: {state: {variety: target_MT}}
        # TOTAL TARGET: 12,375.75 MT (75+4360+136+626+126+100+79+1472+15.75+1000+175+74+0+2114+60+402+1561)
    ...
    def generate_village_summary(self, y0_year: int, y1_year: int) -> pd.DataFrame:
        ...
        for _, vrow in unique_villages.iterrows():
            ...
            for variety in varieties:
                ...
                # Calculate y0_avg_productivity directly from Y0 aggregated data
                y0_avg_productivity = float((y0_packed_qty / y0_net_acres) if y0_net_acres > 0 else 0.0)
                
                # Calculate estimated_cost_per_kg directly from Y0 aggregated data
                ...
                # Determine hybrid type for classification only
    ...
    def allocate_target_production_max(self, village_summary: pd.DataFrame):
        """
        Allocate target production per state-variety using productivity-based allocation (maximize yield).
        Sorts by productivity (descending) and allocates to highest productivity locations first within each state-variety group.
        Only considers villages within the same state for each variety.
        
        ALLOCATION LOGIC (Simple Explanation):
        1. For each state-variety combination (e.g., Odisha-USRH-24):
           - Find all villages in that state growing that variety
           - Sort villages by productivity (yield per acre) from HIGHEST to LOWEST
           - Allocate target production starting with the highest productivity village first
           - Continue allocating until target is met or all villages are used
        
        2. This maximizes total yield by prioritizing the best-performing locations
        
        3. Output values are in KILOGRAMS (Kgs)
           - Targets are stored in MT but converted to Kgs (MT * 1000) for allocation
           - All production values in the output CSV are in Kgs
        """
        ...
        logger.info(f"Starting allocation for {len(self.state_variety_targets_kgs)} state(s) with targets")
        for state, variety_targets in self.state_variety_targets_kgs.items():
            logger.info(f"Processing state '{state}' with {len(variety_targets)} variety target(s)")
            for variety_us_code, target_kgs in variety_targets.items():
                logger.info(f"  Allocating for state='{state}', variety='{variety_us_code}', target={target_kgs} kgs")
                
                # Normalize state name for matching
                normalized_target_state = self.normalize_state_name(state)
                logger.debug(f"    Normalized state: '{state}' -> '{normalized_target_state}'")
                ...
                # Try both exact match and normalized match for state
                ...
                logger.info(
                    f"    Found {len(state_villages)} villages in state '{state}' (normalized='{normalized_target_state}')"
                )
                ...
                if state_villages.empty:
                    logger.warning(
                        f"No villages found for state='{state}' (normalized='{normalized_target_state}'). "
                        f"Total villages in summary: {len(village_summary)}"
                    )
                    if 'state' in village_summary.columns:
                        unique_states = village_summary['state'].dropna().unique()
                        logger.warning(f"Available states in village_summary: {list(unique_states)}")
                    state_variety_stats.append({
                        ...
                        'allocated_acres': 0.0,
                        'total_cost': 0.0,
                        'num_villages_allocated': 0,
                        'num_villages_not_allocated': 0,
                        'num_villages_total': 0,
                ...
                if not matching_villages:
                    logger.warning(
                        f"No matching variety found for state='{state}', variety='{variety_us_code}'. "
                        f"Found {len(state_villages)} villages in state, but none match variety."
                    )
                    if 'variety' in state_villages.columns:
                        unique_varieties = state_villages['variety'].dropna().unique()
                        logger.warning(f"Available varieties in state '{state}': {list(unique_varieties[:10])}")  # Show first 10
                    state_variety_stats.append({
                        'state': state,
                        'variety': variety_us_code,
                        'target_kgs': target_kgs,
                        ...
                        'num_villages_not_allocated': 0,
                        'num_villages_total': 0,
                        'non_allocation_reason': f'No matching variety found (looking for {variety_us_code})',
                ...
                # Filter planned villages with positive productivity and available acres
                planned_mask = (
                ...
                planned_villages_for_allocation = state_variety_villages[planned_mask].copy()
                
                # Store original indices for updating village_summary later
                original_indices = planned_villages_for_allocation.index.tolist()
                
                if planned_villages_for_allocation.empty:
                    state_variety_stats.append({
                        'state': state,
                        'variety': variety_us_code,
                        'target_kgs': target_kgs,
                        ...
                        'total_cost': 0.0,
                        'num_villages_allocated': 0,
                        'num_villages_not_allocated': len(state_variety_villages),
                        'num_villages_total': len(state_variety_villages),
                        'non_allocation_reason': 'No planned villages with positive productivity and available acres',
                        'status': 'No planned villages with positive productivity'
                    })
                    continue
                
                # Sort by productivity (descending) - highest productivity first
                # Create a mapping from sorted position to original index
                planned_villages_for_allocation = planned_villages_for_allocation.sort_values(
                    'y0_avg_productivity', 
                    ascending=False
                )
                # After sorting, the index still points to original indices in village_summary
                sorted_original_indices = planned_villages_for_allocation.index.tolist()
                planned_villages_for_allocation = planned_villages_for_allocation.reset_index(drop=True)
                
                # Calculate max possible production for this state-variety
                max_possible_prod = (
                    planned_villages_for_allocation['y0_avg_productivity'] * 
                    planned_villages_for_allocation['available_acres']
                ).sum()
                ...
                print(f"  Villages available: {len(planned_villages_for_allocation)}")
                
                # Allocate to highest productivity locations first within this state-variety
                allocated = 0.0
                total_cost = 0.0
                total_acres = 0.0
                villages_allocated_count = 0  # Track villages that got allocation
                
                # Track reasons for non-allocation
                villages_target_met = 0  # Target already reached, no need for more villages
                villages_zero_productivity = 0  # Zero or negative productivity
                villages_zero_acres = 0  # Zero or negative available acres
                villages_other_reason = 0  # Other reasons (shouldn't happen normally)
                
                villages_processed = 0  # Track how many villages we've processed
                total_available_villages = len(planned_villages_for_allocation)
                
                for idx, row in planned_villages_for_allocation.iterrows():
                    # Check if target is reached before processing this village
                    if allocated >= target:
                        # All remaining villages are not allocated because target is met
                        villages_target_met = total_available_villages - villages_processed
                        break
                    ...
                    # Get values with explicit float conversion
                    available_acres = float(row['available_acres'])
                    productivity = float(row['y0_avg_productivity'])
                    cost_per_kg = float(row['estimated_cost_per_kg'])
                    
                    # Track reasons for skipping
                    if available_acres <= 0:
                        villages_zero_acres += 1
                        continue
                    if productivity <= 0:
                        villages_zero_productivity += 1
                        continue
                    
                    # Calculate maximum possible allocation for this location
                    max_allocation = min(
                        remaining,
                        available_acres * productivity
                    )
                    
                    # Allocate if there's capacity
                    if max_allocation > 0:
                        # Calculate allocated acres
                        allocated_acres_val = max_allocation / productivity if productivity > 0 else 0.0
                        production = float(max_allocation)
                        estimated_production_cost = float(production * cost_per_kg)
                        
                        # Get original index in village_summary (after sorting)
                        orig_idx = sorted_original_indices[idx]
                        
                        # Update the allocated values in village_summary
                        village_summary.loc[orig_idx, 'allocated_acres'] = round(float(allocated_acres_val), 2)
                        ...
                        allocated += float(max_allocation)
                        total_cost += float(estimated_production_cost)
                        total_acres += float(allocated_acres_val)
                        villages_allocated_count += 1  # Count this village as allocated
                    else:
                        # This shouldn't normally happen, but track it
                        villages_other_reason += 1
                
                # Count remaining non-allocated villages after loop (if target wasn't met)
                # This happens if we processed all villages but didn't reach target
                villages_not_allocated_count = total_available_villages - villages_allocated_count
                
                # Total villages = ALL villages matching state+variety (for statistics display)
                # This shows the total count of villages that match the state-variety combination
                total_villages_matching = len(state_variety_villages)
                
                # Build reason string for non-allocation
                reason_parts = []
                if villages_target_met > 0:
                    reason_parts.append(f"Target met: {villages_target_met}")
                if villages_zero_productivity > 0:
                    reason_parts.append(f"Zero productivity: {villages_zero_productivity}")
                if villages_zero_acres > 0:
                    reason_parts.append(f"Zero acres: {villages_zero_acres}")
                if villages_other_reason > 0:
                    reason_parts.append(f"Other: {villages_other_reason}")
                
                non_allocation_reason = "; ".join(reason_parts) if reason_parts else "N/A"
                
                # Record statistics for this state-variety
                # num_villages_total should show ALL villages matching state+variety (for display)
                ...
                state_variety_stats.append({
                    ...
                    'num_villages_allocated': villages_allocated_count,
                    'num_villages_not_allocated': villages_not_allocated_count,
                    'num_villages_total': total_villages_matching,  # All villages matching state+variety (for statistics)
                    'non_allocation_reason': non_allocation_reason,
    ...
    def save_plan_to_database(self, plan_data: pd.DataFrame, plan_year: int, plan_version: str = None) -> bool:
        ...
        Args:
            ...
            plan_version: Version string for the plan (defaults to v1.0-Maximized-Productivity)
        ...
        try:
            ...
            if not plan_version:
                plan_version = f"v1.0-Maximized-Productivity"
    ...
    def generate_plan(self, plan_year: int):
        ...
        final_plan = self.allocate_target_production_max(village_summary)
        ...
        total_target_mt = total_target / 1000
        print(f"- Total Target Production: {total_target:,.0f} Kg ({total_target_mt:.2f} MT)")
        if plan_year == 2024:
            print(f"  (Expected: 10,000 MT)")
        elif plan_year == 2025:
            print(f"  (Expected: 12,375.75 MT)")
        ...
        # Save the plan to database (PRIORITY) and CSV
        plan_version = f"v1.0-Maximized-Productivity"
        logger.info(f"\nSaving plan to database (priority) and CSV...")
        logger.info(f"  Plan Version: {plan_version}")
        logger.info(f"  Plan Year: {plan_year}")
        logger.info(f"  Records to save: {len(final_plan_ordered)}")
        
        # PRIORITY 1: Save to database first
        ...
        if save_success:
            ...
            # Continue to CSV even if database save fails
        
        # PRIORITY 2: Also save to CSV file
        try:
            # Ensure output folder exists
            os.makedirs(self.output_folder, exist_ok=True)
            
            # Generate CSV filename - use plan year range format (e.g., 2024_25 for plan_year 2024)
            plan_name = f"{plan_year}_{str(plan_year + 1)[-2:]}"
            csv_filename = os.path.join(self.output_folder, f"supply_chain_plan_{plan_name}.csv")
            
            # Save to CSV
            final_plan_ordered.to_csv(csv_filename, index=False)
            logger.info(f"Plan also saved to CSV: {csv_filename}")
            print(f"\n[OK] Plan saved to CSV: {csv_filename}")
        except Exception as e:
            logger.warning(f"Warning: Failed to save plan to CSV: {str(e)}")
            print(f"\n[WARNING] Failed to save plan to CSV: {str(e)}")
            # Don't raise - database save is priority, CSV is secondary
    ...
    def print_state_variety_statistics(self, plan_year: int):
        ...
        for state in sorted(self.state_variety_statistics['state'].unique()):
            ...
            print(f"\n{state.upper()}:")
            print("-" * 120)
            print(f"{'Variety':<15} {'Target (MT)':<15} {'Allocated (MT)':<18} {'% Achieved':<12} {'Acres':<12} {'Cost (Rs)':<15} {'Villages':<25} {'Status':<20}")
            print(f"{'':<15} {'':<15} {'':<18} {'':<12} {'':<12} {'':<15} {'(Alloc/Not/Total)':<25} {'':<20}")
            print("-" * 120)
            ...
            state_total_cost = 0
            state_total_acres = 0
            state_total_villages_allocated = 0
            state_total_villages_not_allocated = 0
            state_total_villages_total = 0
            ...
            for _, row in state_data.iterrows():
                ...
                # Get village counts (handle missing columns gracefully)
                villages_allocated = int(row.get('num_villages_allocated', 0) or 0)
                villages_not_allocated = int(row.get('num_villages_not_allocated', 0) or 0)
                villages_total = int(row.get('num_villages_total', 0) or 0)
                villages_str = f"{villages_allocated}/{villages_not_allocated}/{villages_total}"
                
                # Get non-allocation reason
                non_allocation_reason = str(row.get('non_allocation_reason', 'N/A') or 'N/A')
                
                print(f"{row['variety']:<15} {target_mt:>12.2f} {allocated_mt:>15.2f} {pct_achieved:>10.1f}% "
                      f"{row['allocated_acres']:>10.2f} {row['total_cost']:>13,.0f} {villages_str:<25} {row['status']:<20}")
                
                # Print non-allocation reason if there are non-allocated villages
                if villages_not_allocated > 0:
                    print(f"{'':<15} {'':<15} {'':<18} {'':<12} {'':<12} {'':<15} {'':<25} {'Reason: ' + non_allocation_reason[:60]:<20}")
                ...
                state_total_villages_allocated += villages_allocated
                state_total_villages_not_allocated += villages_not_allocated
                state_total_villages_total += villages_total
            
            state_pct = (state_total_allocated / state_total_target * 100) if state_total_target > 0 else 0.0
            state_villages_str = f"{state_total_villages_allocated}/{state_total_villages_not_allocated}/{state_total_villages_total}"
            print("-" * 120)
            print(f"{'TOTAL':<15} {state_total_target:>12.2f} {state_total_allocated:>15.2f} {state_pct:>10.1f}% "
                  f"{state_total_acres:>10.2f} {state_total_cost:>13,.0f} {state_villages_str:<25}")
        ...
        # Calculate total village counts
        total_villages_allocated = int(self.state_variety_statistics.get('num_villages_allocated', pd.Series([0])).sum())
        total_villages_not_allocated = int(self.state_variety_statistics.get('num_villages_not_allocated', pd.Series([0])).sum())
        total_villages_total = int(self.state_variety_statistics.get('num_villages_total', pd.Series([0])).sum())
        ...
        print(f"\nVillage Statistics:")
        print(f"  Villages Allocated: {total_villages_allocated}")
        print(f"  Villages Not Allocated: {total_villages_not_allocated}")
        print(f"  Total Villages Available: {total_villages_total}")
        
        # Parse and summarize non-allocation reasons
        if total_villages_not_allocated > 0 and 'non_allocation_reason' in self.state_variety_statistics.columns:
            print(f"\nReasons for Non-Allocation (Summary):")
            import re
            reason_counts = {
                'Target met': 0,
                'Zero productivity': 0,
            ...
            for _, row in self.state_variety_statistics.iterrows():
                reason = str(row.get('non_allocation_reason', '') or '')
                if 'Target met' in reason:
                    match = re.search(r'Target met:\s*(\d+)', reason)
                    if match:
                        reason_counts['Target met'] += int(match.group(1))
                if 'Zero productivity' in reason:
                    match = re.search(r'Zero productivity:\s*(\d+)', reason)
                    if match:
                        reason_counts['Zero productivity'] += int(match.group(1))
                if 'Zero acres' in reason:
                    match = re.search(r'Zero acres:\s*(\d+)', reason)
                    if match:
                        reason_counts['Zero acres'] += int(match.group(1))
                if 'Other' in reason:
                    match = re.search(r'Other:\s*(\d+)', reason)
                    if match:
                        reason_counts['Other'] += int(match.group(1))
            
            for reason, count in reason_counts.items():
                if count > 0:
                    print(f"  {reason}: {count} villages")
        
        print("=" * 100)
        
        # Save statistics to CSV
        stats_filename = os.path.join(self.output_folder, f"state_variety_statistics_{plan_year}.csv")
    ...
    def execute_with_ui_selections(
        ...
        logger.info(f"Processing state_variety_targets from UI payload: {list(state_variety_targets.keys())} states")
        
        for state, varieties in state_variety_targets.items():
            self.state_variety_targets[state] = {}
            self.state_variety_targets_kgs[state] = {}
            logger.info(f"  Processing state '{state}' with {len(varieties)} variety(ies)")
            ...
            for variety_id, target_mt in varieties.items():
                ...
                logger.info(
                    f"    Converting: variety_id={variety_id} -> variety_name={variety_name} -> "
                    f"normalized={normalized_variety}, target={target_mt} MT"
                )
                ...
                if target_mt > 0:
                    ...
                    logger.warning(f"    Skipping {variety_id} with zero target: {target_mt} MT")
        
        logger.info(f"Final self.state_variety_targets structure: {self.state_variety_targets}")
        logger.info(f"Final self.state_variety_targets_kgs structure: {self.state_variety_targets_kgs}")
        ...
        def print_stats_no_csv(plan_year_arg):
            ...
            for state in sorted(self.state_variety_statistics['state'].unique()):
                ...
                for _, row in state_data.iterrows():
                    ...
                    allocated_mt = row['allocated_mt']
                    pct_achieved = (allocated_mt / target_mt * 100) if target_mt > 0 else 0.0
                    villages_str = f"{int(row.get('num_villages_allocated', 0))}/{int(row.get('num_villages_not_allocated', 0))}/{int(row.get('num_villages_total', 0))}"
                    print(f"{row['variety']:<15} {target_mt:>12.2f} {allocated_mt:>15.2f} {pct_achieved:>10.1f}% "
                          f"{row['allocated_acres']:>10.2f} {row['total_cost']:>13,.0f} {villages_str:<25} {row['status']:<20}")
        ...
        try:
            logger.info(f"Starting generate_plan execution for plan_year={plan_year}")
            logger.info(f"Targets structure: {self.state_variety_targets}")
            
            # Ensure data is loaded (same as original planner)
            # generate_village_summary() uses self.data, so we need to ensure it's loaded
            if not hasattr(self, 'data') or self.data.empty:
                logger.info("Reloading data from database for UI execution")
                self.load_from_database()
            
            # Call generate_plan with overridden targets - uses exact same allocation logic
            # generate_plan() calls generate_village_summary() which includes ALL villages (no pre-filtering)
            # Then allocate_target_production_max() filters by state-variety targets
            result_df = self.generate_plan(plan_year)
            
            logger.info(f"generate_plan returned {len(result_df)} rows")
            if not result_df.empty:
                logger.info(f"Result columns: {list(result_df.columns)}")
                if 'state' in result_df.columns:
                    logger.info(f"Unique states in result: {result_df['state'].unique()}")
                if 'variety' in result_df.columns:
                    logger.info(f"Unique varieties in result: {result_df['variety'].unique()}")
...
def main():
    """
    Main function to generate productivity-based supply chain plans
    Loads data from database (schema: operations) only
    """
    print("=" * 80)
    print("SUPPLY CHAIN PLANNING - PRODUCTIVITY MAXIMIZATION (v1.0)")
    ...
    try:
        ...
        try:
            ...
            # Verify saved plans in database (PRIORITY)
            print("\n" + "=" * 80)
            print("VERIFYING SAVED PLANS IN DATABASE (PRIORITY)")
            print("=" * 80)
            
            for plan_year in [2024, 2025]:
                verification = planner.verify_saved_plan(plan_version="v1.0-Maximized-Productivity", plan_year=plan_year)
                if verification['success']:
                    print(f"\nYear {plan_year}: Found {verification['total_records']} records in database")
                    if verification.get('plan_version'):
                        print(f"   Version: {verification['plan_version']}")
            ...
            # Display CSV file locations (SECONDARY)
            print("\n" + "=" * 80)
            print("CSV FILES GENERATED (SECONDARY)")
            print("=" * 80)
            
            output_folder = planner.output_folder
            print(f"\nOutput folder: {output_folder}")
            
            for plan_year in [2024, 2025]:
                plan_name = f"{plan_year}_{str(plan_year + 1)[-2:]}"
                csv_filename = os.path.join(output_folder, f"supply_chain_plan_{plan_name}.csv")
                if os.path.exists(csv_filename):
                    file_size = os.path.getsize(csv_filename) / 1024  # Size in KB
                    print(f"\nYear {plan_year} Plan CSV:")
                    ...
                    print(f"\nYear {plan_year} Plan CSV: File not found")
            
            print("\n" + "=" * 80)
            print("PLANNING COMPLETED")
            print("=" * 80)
            print(f"\nPlans saved to:")
            print(f"  1. Database (PRIORITY): operations.supply_chain_planning table")
            print(f"  2. CSV files (SECONDARY): {output_folder}")
            print("\n To verify database manually:")
            print("   SELECT COUNT(*) FROM operations.supply_chain_planning WHERE plan_revision_version = 'v1.0-Maximized-Productivity';")
            print("   SELECT * FROM operations.supply_chain_planning WHERE plan_revision_version = 'v1.0-Maximized-Productivity' ORDER BY created_at DESC LIMIT 10;")
